class Api::FavoritesController < ApplicationController
  def index
    @favorites = FavoriteDecorator.decorate_collection(
      Favorite.where(user_id: current_user.id).limit(20)
    )
    render 'index', formats: 'json', handlers: 'jbuilder'
  end

  def exists?
    is_favorite = Favorite.exists?(book_id: params[:book_id], user_id: current_user.id)
    render json: { exists: is_favorite }
  end

  def create
    Favorite.create(favorite_params.merge({ user_id: current_user.id }))
  ...
  def destroy
    Favorite.delete(book_id: params[:book_id], user_id: current_user.id)
